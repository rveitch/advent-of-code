...
function decodePriority(val) {
  return priorityKey.indexOf(val) + 1;
}

function computeSharedPriority(sharedPriorities) {
  return sharedPriorities.reduce((totalScore, encodedPriority) => {
    const decodedPriority = decodePriority(encodedPriority);
...
function solvePartOne() {
  return ptOneInput.reduce((totalScore, contents) => {
...
function solvePartTwo() {
  const groupedRucksacks = _.chunk(ptOneInput, 3).map((groups) => {
